...
export function Navbar() {
  ...
  const isLightTopPage =
    ...
    location.pathname === '/tamil-nadu-tour-packages' ||
    location.pathname === '/kerala-tour-packages' ||
    location.pathname === '/karnataka-tour-packages';
  const effectiveScrolled = scrolled || isLightTopPage;

  return (
    <header
      className={`fixed z-50 transition-all duration-500 mx-auto left-0 right-0 max-w-7xl ${effectiveScrolled
        ...
        <ul className="hidden items-center gap-8 lg:flex">
          {links.map((link) => (
            <li key={link.label} className="relative group">
              <Link
                to={link.href}
                className={`flex items-center gap-1 text-base font-bold transition-colors duration-200 tracking-wide ${effectiveScrolled
            ...
            {links.map((link) => (
              <li key={link.label}>
                <div className="flex items-center justify-between">
                  <Link
                    to={link.href}
                    className={`block text-lg font-bold ${effectiveScrolled
                      ? location.pathname === link.href
                        ? 'text-[var(--color-blue-ocean)]'
