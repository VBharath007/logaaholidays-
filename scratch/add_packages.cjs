...
// Find the last package in packagesDatabase and append
let packagesString = '';
for(let key in packages) {
    let p = packages[key];
    packagesString += `,\n    '${key}': ` + JSON.stringify(p, null, 8).replace(/\"/g, "'").replace(/'([^']+)':/g, (match, p1) => {
        if (/^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(p1)) return `${p1}:`;
        return match;
    });
}
// Clean up JSON.stringify indenting to match tsx manually
packagesString = packagesString.replace(/\n        /g, '\n        ');

content = content.replace("South India tour package, Logaa Holidays Karnataka'\n    }\n};", "South India tour package, Logaa Holidays Karnataka'\n    }" + packagesString + "\n};");
