...
for i in range(1, len(parts), 2):
    ...
    if match:
        ...
        if title in manual_hero_map:
            ...
            # If heroImage exists, replace it
            if '"heroImage"' in rest:
                rest = re.sub(r'("heroImage":\s*)[\'"][^\'"]+[\'"]', r'\1"' + hero_path + '"', rest, count=1)
            else:
                # If it doesn't exist, inject it right after the image field
                rest = re.sub(r'("image":\s*[\'"][^\'"]+[\'"],)', r'\1\n    "heroImage": "' + hero_path + '",', rest, count=1)
            
    new_content += prefix + rest

with open(file, 'w', encoding='utf-8') as f:
    f.write(new_content)
print("Added missing heroImages and updated existing ones!")
