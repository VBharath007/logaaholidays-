...
const startIndex = fileContent.indexOf("  'madurai-tourism': {");
const endIndex = fileContent.indexOf("  'kerala-tourism': {");

if (startIndex === -1 || endIndex === -1) {
  console.log('Could not find boundaries for madurai-tourism');
  process.exit(1);
}

const newMaduraiBlock = `  'madurai-tourism': {
    id: 'madurai-tourism',
    name: 'Madurai',
...
const updatedContent = fileContent.substring(0, startIndex) + newMaduraiBlock + fileContent.substring(endIndex);

fs.writeFileSync('src/data/destinationsData.ts', updatedContent, 'utf8');
console.log('Successfully replaced Madurai block with corrected data');
