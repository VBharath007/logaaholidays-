...
with open('public/extracted_chennai_varanasi.txt', 'r', encoding='utf-8') as f:
    content = f.read()
...
for i, line in enumerate(lines):
    stripped = line.strip()
    if re.match(r'^(?:[\U0001F300-\U0001FFFF\u2600-\u26FF\u2700-\u27BF]\s*)?(?:Chennai|Madurai) to .*(?:Package|Tour)', stripped, re.I):
        if '|' in stripped:
...
for pkg_idx, start_line in enumerate(package_starts):
    ...
    day_blocks = re.split(r'(Day\s*\d+\s*[\u2013\-\u2014].*)', block)
    if len(day_blocks) > 1:
